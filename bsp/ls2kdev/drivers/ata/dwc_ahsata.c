...
{
	u64 dwc_ahsata_base = 0;
	struct pci_header *p = (struct pci_header *)(0x9000000000000000 | SATA_BASE);
	dwc_ahsata_base = (0x9000000000000000 | (p->BaseAddressRegister[0] & 0xfffffff00));
	...
	ahci_device -> mmio_base = (void *)dwc_ahsata_base;
	...
	rt_kprintf("rt_hw_ahci_host_init ....\n");
	...
	rt_kprintf("rt_hw_ahci_host_init end....\n");
...
int dwc_ahsata_scan(struct rt_device *dev)
{
	...
	rt_kprintf("dwc_ahsata_scan ret:%d\n", ret);
	if(ret != RT_EOK)
	{
